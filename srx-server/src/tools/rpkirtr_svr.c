...
bool sendAspaPdu_Upstream(int* fdPtr, u_int8_t version)
{
  bool retVal = true;
  int headerSize = sizeof(RPKIAspaHeader);
  uint8_t                  aspaHeader[headerSize];
...
#define UP_STREAM_COUNT 6
  ...
  {
    hdr->customerAsn = htonl(asns[i]); // test
    pasn = htonl(asns[i+1]);

    memcpy(pdu, aspaHeader, headerSize);
    memcpy(pdu+headerSize, &pasn, remain);
...
bool sendAspaPdu_Downstream1(int* fdPtr, u_int8_t version)
{
  bool retVal = true;
  int headerSize = sizeof(RPKIAspaHeader);
  uint8_t                  aspaHeader[headerSize];
  ...
  //
#define UP_COUNT 5
  uint32_t asns[UP_COUNT] = {40, 30, 20, 10, 0};
  uint32_t casn, pasn;
...
bool sendAspaPdu_Downstream2(int* fdPtr, u_int8_t version)
{
  bool retVal = true;
  int headerSize = sizeof(RPKIAspaHeader);
  uint8_t                  aspaHeader[headerSize];
...
#define DOWN_COUNT 4
  // TODO: here need to obtain or manipulate providerASNs
  //
  uint32_t asns[DOWN_COUNT ] = {60001, 60002, 60004, 10};
  uint32_t casn, pasn;
